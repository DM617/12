...
void vector::add_element(complex& V) {
	vect.push(V);
}

void vector::add(vector& S) {
	int firstd, secondd;
	firstd = vect.dimension();
	secondd = S.vect.dimension();
	if (firstd > secondd) {
		complex complex_null;
		for (int i = 0; i < (firstd - secondd); i++) {
			S.vect.push(complex_null);
		}
	}
